...
class DashboardViewModel @Inject constructor(
    ...
    private fun splitIfMultiTech(token: String): List<String> {
        ...
        if (token.contains(',') || token.contains('•') || token.contains('·') || token.contains(';')) return listOf(token)
        val parts = token.split(Regex("\\s+"))
        // Heuristic: split only if it looks like concatenated standalone techs (>=3 capitalized words)
        return if (parts.size >= 3 && parts.all { it.firstOrNull()?.isUpperCase() == true }) parts else listOf(token)
    }
}
