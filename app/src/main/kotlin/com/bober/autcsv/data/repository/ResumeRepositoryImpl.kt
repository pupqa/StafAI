...
sealed class Result<out T> {
    ...
    /** Ошибка с исключением для диагностики. */
...
class ResumeRepositoryImpl @Inject constructor(
    ...
    /**
     * Загружает одно резюме по ID, логирует обращение к БД и оборачивает исключения.
     */
    ...
    /**
     * Возвращает поток резюме и конвертирует сущности БД в доменные модели.
     */
    ...
    /**
     * Сохраняет резюме, конвертируя доменную модель в сущность Room.
     */
    ...
    /**
     * Генерирует PDF в кэше приложения через выбранный шаблон, с логированием времени.
     */
    override suspend fun generatePdf(resume: Resume, templateType: PdfTemplateType): String =
        withContext(Dispatchers.IO) {
            try {
                ...
                // Создаем файл PDF в кэше приложения
                val pdfFile = File(context.cacheDir, "${resume.id}.pdf")

                // Генерируем PDF с использованием выбранного шаблона
    ...
    /** Запускает анализ резюме через сервис OpenRouter. */
