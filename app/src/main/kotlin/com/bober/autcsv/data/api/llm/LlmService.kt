...
/**
 * Сервис для работы с OpenAI-клиентом через OpenRouter-конфигурацию.
 * Формирует промпты и парсит ответы в доменные модели.
 */
@Singleton
class LlmService @Inject constructor(
    ...
    /** Анализирует резюме и возвращает модель [AiAnalysis]. */
    ...
    /** Возвращает список текстовых рекомендаций по улучшению резюме. */
    ...
    /** Улучшает текст описания (проект/достижение) с упором на конкретику. */
    suspend fun enhanceDescription(text: String): String {
        val prompt = buildEnhancementPrompt(text)
        return getCompletion(prompt)
    }

    /** Выполняет чат-запрос к модели и возвращает контент первого ответа. */
    ...
    /** Формирует промпт для общего анализа резюме. */
    ...
    /** Формирует промпт для генерации улучшений резюме. */
    ...
    /** Формирует промпт для улучшения конкретного текстового описания. */
    ...
    /** Безопасно парсит JSON-ответ анализа в [AiAnalysis]. */
    ...
    /** Разбивает ответ на строки и фильтрует пустые — простой парсер рекомендаций. */
