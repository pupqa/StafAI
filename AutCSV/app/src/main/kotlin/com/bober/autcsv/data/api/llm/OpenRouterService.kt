...
/**
 * Сервис-обертка над [OpenRouterApi] для анализа резюме через LLM.
 *
 * Замечания:
 * - В текущем виде содержит встроенный ключ API (для продакшена требуется безопасное хранилище).
 * - Имеет стратегию перебора моделей по приоритету и graceful degradation.
 * - Возвращает обновленную доменную модель [Resume] с результатами анализа.
 */
@Singleton
class OpenRouterService @Inject constructor(
    ...
    private val apiKey = "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"

    /**
     * Анализирует резюме, последовательно пытаясь использовать разные модели.
     * При успешном ответе парсит и возвращает копию резюме с заполненным `aiAnalysis`.
     *
     * @throws IllegalStateException если все модели недоступны или произошла фатальная ошибка.
     */
    suspend fun analyzeResume(resume: Resume): Resume {
        ...
        throw IllegalStateException("Все доступные модели недоступны. Последняя ошибка: ${lastException?.message}", lastException)
    }

    /**
     * Выполняет фактический HTTP-запрос к OpenRouter для конкретной модели.
     * Формирует промпт из содержимого резюме, отправляет запрос и парсит ответ.
     * Возвращает обновленную копию [Resume] с полем `aiAnalysis`.
     */
    private suspend fun analyzeResumeWithModel(resume: Resume, modelId: String): Resume {
        ...
        val response = try {
            ...
            when (e.code()) {
                ...
                401 -> throw IllegalStateException("Неверный API ключ OpenRouter. Проверьте настройки.", e)
                429 -> throw IllegalStateException("Превышен лимит запросов. Попробуйте позже.", e)
                else -> throw IllegalStateException("Ошибка при анализе резюме с моделью $modelId: HTTP ${e.code()}", e)
            }
        } catch (e: Exception) {
            throw IllegalStateException("Ошибка при анализе резюме с моделью $modelId: ${e.message}", e)
        }
    ...
    /**
     * Грубый парсер метрики полноты из текста ответа LLM.
     * Ищет строку со словом "score" и извлекает цифры.
     */
    ...
    /**
     * Извлекает списки улучшений из маркдаун-подобного ответа.
     * Поддерживает строки, начинающиеся с "-" или с нумерации вида "1.".
     */
