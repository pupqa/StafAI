...
/**
 * Доменная модель результата анализа резюме.
 *
 * Содержит интегральные показатели (рейтинг, полнота), текстовые выводы,
 * а также структурированные списки сильных сторон, улучшений и рекомендаций.
 */
data class CvAnalysis(
    ...
    companion object {
        /**
         * Парсит строковый ответ LLM в структуру [CvAnalysis].
         * Выполняет эвристическое извлечение блоков текста и списков по ключевым словам.
         * При отсутствии явных маркеров использует простые fallback-стратегии.
         */
        fun fromLlmResponse(content: String): CvAnalysis {
                ...
                for (keyword in keywords) {
                    ...
                    if (startIndex != -1) {
                        val result = lines.drop(startIndex + 1)
                            ...
                            .joinToString("\n")
                            .trim()
                        LlmLogger.logAnalysisResult("Extracted section for '$keyword'", "Length: ${result.length}")
                        return result
                    }
                }
                LlmLogger.logAnalysisResult("No section found for keywords", keywords.joinToString(", "))
                return ""
            }
            ...
            fun extractList(keywords: List<String>): List<String> {
                for (keyword in keywords) {
                    ...
                    if (startIndex != -1) {
                        ...
                        LlmLogger.logAnalysisResult("Extracted list for '$keyword'", "Items: ${result.size}")
                        return result
                    }
                }
                LlmLogger.logAnalysisResult("No list found for keywords", keywords.joinToString(", "))
                return emptyList()
            }
            
            // Extract completeness analysis (rating section)
            val completenessAnalysis = extractSection(listOf(
                "Общая оценка", "Overall Assessment", "Оценка полноты", "Rating", "1."
            ))
            
            // Extract logic analysis (improvements section)
            val logicAnalysis = extractSection(listOf(
                "Зоны для улучшения", "Areas for Improvement", "Слабые стороны", "Weak Areas", "3."
            ))
            
            // Extract strengths
            val strengths = extractList(listOf(
                "Сильные стороны", "Strengths", "Сильные стороны:", "Strengths:", "2."
            ))
            
            // Extract improvements
            val improvements = extractList(listOf(
                "Зоны для улучшения", "Areas for Improvement", "Слабые стороны", "Weak Areas", "3."
            ))
            
            // Extract recommendations
            val recommendations = extractList(listOf(
                "Рекомендации", "Recommendations", "Конкретные рекомендации", "Specific recommendations", "4."
            ))
            ...
            val fallbackCompleteness = if (completenessAnalysis.isBlank()) {
                content.lines()
                    .takeWhile { !it.contains("сильные стороны", ignoreCase = true) && !it.contains("strengths", ignoreCase = true) }
                    .joinToString("\n")
                    .trim()
            ...
            val fallbackLogic = if (logicAnalysis.isBlank()) {
                content.lines()
                    .dropWhile { !it.contains("зоны для улучшения", ignoreCase = true) && !it.contains("areas for improvement", ignoreCase = true) }
                    .takeWhile { !it.contains("рекомендации", ignoreCase = true) && !it.contains("recommendations", ignoreCase = true) }
                    .joinToString("\n")
                    .trim()
            } else logicAnalysis
            ...
            LlmLogger.logAnalysisResult("Parsed CvAnalysis", "Rating: ${result.rating}, Completeness: ${result.completenessAnalysis.length}, Logic: ${result.logicAnalysis.length}, Strengths: ${result.strengths.size}, Improvements: ${result.improvements.size}, Recommendations: ${result.recommendations.size}, CompletenessScore: ${result.completenessScore}")
            
            return result
        }
        
        /**
         * Извлекает числовую метрику полноты в диапазоне 0..100 из текста.
         */
        ...
        /**
         * Пытается извлечь дополнительные параметризованные показатели качества.
         * Поиск ведется по эвристическим ключевым словам и разделителю ':'.
         */
        private fun extractAdditionalParameters(content: String): Map<String, String> {
            ...
            val paramKeywords = mapOf(
                ...
                "Соответствие специализации" to listOf("соответствие", "специализация", "релевантность"),
